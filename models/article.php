...
class article_class extends AWS_MODEL
{
    ...
    public function get_by_like($q, $page = 1, $limit = 20, $is_recommend = false)
    {
        $offset = ($page-1) * $limit;
        // var_dump($q);
        $sql = "select * from aws_article where title regexp'".$this->quote(implode('|', $q))."' or message regexp'".$this->quote(implode('|', $q))."' order by add_time desc limit ".calc_page_limit($page,$limit);
